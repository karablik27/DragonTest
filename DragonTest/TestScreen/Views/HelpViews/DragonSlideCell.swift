...
final class DragonSlideCell: UICollectionViewCell {
    ...
    func configure(with test: DragonTest) {
        // фон у каждой карточки
        GradientBackground.attach(to: contentView, colors: test.dragon.gradientColors)
        
        // дракон через кэш (без лагов)
        if let entity = DragonCache.shared.clone(for: test.dragon, scale: [0.8,0.8,0.8]) {
            preview.displayEntity(entity)
        }
        
        titleLabel.text = test.dragon.title
        infoLabel.text = "Вопросов: \(test.totalQuestions) • Пройдено: \(test.completed)"
    }
}
